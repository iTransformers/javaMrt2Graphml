...
//			}
    ...
    public static void dumpNodes(ASContainer ases, Writer writer, String tabs) throws IOException {
        ...
        for (ASInfo asInfo : ases.getAsInfoMap().values()) {
            ...
            List<PrefixInfo> prefixInfoList = asInfo.getPrefixInfo();
            Map<String, Integer> ipvXcounter = countIpVXPrefixes(prefixInfoList);
            Map<String, Long> ipvXAddressSpace = countIpVXAdressSpace(prefixInfoList);
            writer.write(tabs+"\t<data key=\"IPv4AddressSpace\">"+ ((ipvXAddressSpace.get(IPV4_KEY)>0) ? ipvXAddressSpace.get(IPV4_KEY):0)+"</data>\n");
            writer.write(tabs+"\t<data key=\"IPv6AddressSpace\">"+ ((ipvXAddressSpace.get(IPV6_KEY)>0) ? ipvXAddressSpace.get(IPV6_KEY):0) +"</data>\n");
            writer.write(tabs+"\t<data key=\"IPv4PrefixCount\">"+ ((ipvXcounter.get(IPV4_KEY)>0) ? ipvXcounter.get(IPV4_KEY):0) +"</data>\n");
            writer.write(tabs+"\t<data key=\"IPv6PrefixCount\">"+ ((ipvXcounter.get(IPV6_KEY) > 0) ? ipvXcounter.get(IPV6_KEY):0) +"</data>\n");
            ...
            writer.write(tabs+"\t<data key=\"IPv4Prefixes\">");
            for (int i = 0; i < prefixInfoList.size();i++){
                String prefix =prefixInfoList.get(i).getPrefix();
                if(prefix.contains(".")){
                    writer.write(prefix+",");
                }
            }
            writer.write("</data>\n");
            writer.write(tabs+"\t<data key=\"IPv6Prefixes\">");
            for (int i = 0; i < prefixInfoList.size();i++){
                String prefix =prefixInfoList.get(i).getPrefix();
                if(prefix.contains(":")){
                    writer.write(prefix+",");
    ...
    private static List<PrefixInfo> getIpXPrefixes(List<PrefixInfo> prefixInfoList,String prefixType) {
        List<PrefixInfo> result = new ArrayList<PrefixInfo>();

        for (PrefixInfo prefixInfo : prefixInfoList) {
            String prefix = prefixInfo.getPrefix();

            if(prefix.contains(".") && "IPv4".equals(prefixType)){
                result.add(prefixInfo);
            } else{
                result.add(prefixInfo);
            }
        }
	...
	private static void usage() {
		...
		ps.println("  -o as     output to graphml");
        ps.println("  -z as     generate zip file");
